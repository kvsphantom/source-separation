...
input_dir='/home/kvshenoy/project/dataset/juan/fps0.01'
split_dir='/home/kvshenoy/project/dataset/juan/dummy'
...
for x in X_train:
    ...
    save_to_path=os.path.join(split_dir, 'train', category, os.path.basename(x))
    create_folder(os.path.dirname(save_to_path))
    copyfile(x,save_to_path)
for x in X_val:
    category = os.path.basename(os.path.dirname(x))
    save_to_path = os.path.join(split_dir, 'val', category, os.path.basename(x))
    create_folder(os.path.dirname(save_to_path))
    copyfile(x, save_to_path)